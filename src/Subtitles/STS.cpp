...
#ifdef USE_LIBASS
    UnloadASS();
    if (s.fRenderUsingLibass) { //styles may change the way the libass file was loaded, so we reload it here
        usingLibass = true;
        LoadASSFile(m_subtitleType);
    } else {
        usingLibass = false;
...
void STSStyle::SetDefault()
{
    ...
    openTypeLangHint[0] = 0;
    fRenderUsingLibass = false;
    fontName = _T("Tahoma");
    fontSize = 18;
...
bool STSStyle::IsFontStyleEqual(const STSStyle& s) const
{
    return (
               charSet == s.charSet
               && CStringA(openTypeLangHint) == CStringA(s.openTypeLangHint)
               && fRenderUsingLibass == s.fRenderUsingLibass
               && fontName == s.fontName
               && fontSize == s.fontSize
...
{
    style.Format(_T("%d;%d;%d;%d;%d;%d;%f;%f;%f;%f;0x%06lx;0x%06lx;0x%06lx;0x%06lx;0x%02x;0x%02x;0x%02x;0x%02x;%d;%s;%f;%f;%f;%f;%ld;%d;%d;%d;%d;%f;%f;%f;%f;%d;%hs;%d"),
                 ...
                 s.relativeTo, s.openTypeLangHint, s.fRenderUsingLibass);
